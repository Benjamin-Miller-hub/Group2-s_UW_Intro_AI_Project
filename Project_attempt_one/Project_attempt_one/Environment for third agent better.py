...
def main():
    ...
    state,reward,done,info = test.step(0,1,2,1,2)
    state,reward,done,info = test.step(0,1,2,2,1)
    state,reward,done,info = test.step(0,1,2,1,2)
    state,reward,done,info = test.step(0,1,3,1,2)
    state,reward,done,info = test.step(1,4,3,1,2)
    state,reward,done,info = test.step(1,4,3,1,2)
    state,reward,done,info = test.step(1,4,3,1,2)
    print("Checking")
...
class ConnectFour(Env):
    ...
    def total_reward_function(self,row_inserted,action,actual_position,player,opponent):
        ...
        positive_reward,done_pos = self.reward_function_positive(row_inserted,action,player)
