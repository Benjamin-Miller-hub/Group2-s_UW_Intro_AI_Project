...
def main():
    ...
    game = Environment_for_first_agent_better.ConnectFour(player_one,player_two)
    turn_player = player_one
    action_steps = 0
    for epoch_count in range(epochs):
       if epoch_count % 2 == 0:
            state = game.reset(player_one,player_two)
            state = state[0]
       else:
            state = game.reset(player_two,player_one)
       ...
       for time_step in range(max_steps_per_episode):
            ...
            next_state,reward,done,_ = game.step(action,turn_player)
